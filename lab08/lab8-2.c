  ...
  else
  {
    ...
    do
    {
		...
		if (time > counter + buffer){ 
			...
			if(MAZE[c][r -1] != WALL){ // Checks space on top
				draw_character(c, r - 1, EMPTY_SPACE);
			}
			if(MAZE[c + 1][r] != WALL){ // Checks space on right
				draw_character(c + 1, r, EMPTY_SPACE);
			}	
			if(MAZE[c -1][r] != WALL){ // Checks space on left
				draw_character(c - 1, r, EMPTY_SPACE);
			}
			if(MAZE[c + 1][r - 1] != WALL){ // Checks space on top right
				draw_character(c + 1, r - 1, EMPTY_SPACE);
			}
			if(MAZE[c - 1][r - 1] != WALL){ // Checks space on top left
				draw_character(c - 1, r - 1, EMPTY_SPACE);
			}
			if(MAZE[c][r + 1] != WALL){ // Check space to go down
				r++;
			}
			...
			if (calc_roll(new_x) == 1.0 && (MAZE[c - 1][r] == ' ')){	// Left
					draw_character(c, r, ' ');
					c = c - 1;
					draw_character(c, r, 'A');
			}
				if (calc_roll(new_x) == - 1.0 && (MAZE[c + 1][r] == ' ')){		//Right
					draw_character(c, r, ' ');
					c = c + 1;
					draw_character(c, r, 'A');
				}
