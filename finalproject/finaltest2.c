...
int main(int argc, char* argv[]) {
	...
	//wordcount = sizeof(wordlist); <--- FIX ME: This is what causes the word count to be 16000
	...
	while (userChar == 'w'){ 													// FIX ME: Condition to enter word. Need to add condition to exit loop, since it will repeat itself no matter what key is entered.

		printf("You currently have %d words that can be used.\n", wordcount);	// FIX ME: prints twice

				printf("What word would you like to add?\n"); 					// FIX ME: prints twice leaving an empty space, then works perfectly
					fp = fopen("wordList2.txt", "a"); 							// Opens file in append mode
						
						if (fp == NULL){										// Exits if text file does not exist
							printf("Failed to open file\n");
							exit(0);
						}
						else {
							fgets(line, sizeof(line),stdin); 					// Allows you to write word
							fprintf(fp, "%s", line); 							// Sends word to file
							wordcount++; 										// Updates word count
							 }	
	...
	while (userChar == 'p'){ 													// Condition to enter game
